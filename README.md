...
After a session has been idle for four minutes, pi-fleet starts a separate no-tools Pi call to produce a two-line recap. That call receives a truncated excerpt of the last eight messages and uses `deepseek/deepseek-v4-flash` by default. Disable or modify this behaviour before using pi-fleet in sensitive conversations.
